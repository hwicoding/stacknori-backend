...
- **CI/CD 테스트 자동화**: `.github/workflows/test.yml` 생성, push/PR 시 자동으로 pytest 실행 및 커버리지 리포트 생성 (Codecov 연동)
- **Alembic 자동화 구현**: `scripts/docker-entrypoint.sh` 생성, 컨테이너 시작 시 PostgreSQL 준비 대기 후 자동 마이그레이션 실행
- **Dockerfile 개선**: dev/prod 타겟에 entrypoint 스크립트 추가, `postgresql-client` 패키지 설치로 DB 연결 확인 가능
- **GitHub Actions 마이그레이션 검증**: 테스트 워크플로우에 `alembic check` 및 `alembic history` 단계 추가하여 마이그레이션 무결성 검증
- **초기 Admin/Seed 전략 구현**: `scripts/seed_admin.py` 생성, 환경 변수 기반 초기 관리자 계정 생성 스크립트
- **Role 시스템 설계 문서화**: `docs/ROLE_SYSTEM.md`에 현재 구현 상태, 향후 확장 계획, 권한 매트릭스 정리
- **관리자 권한 체크 Dependency**: `app/core/dependencies.py`에 `get_current_admin_user` 추가, 관리자 전용 API 엔드포인트 보호
- **Role Enum 정의**: `app/domain/entities/role.py`에 `UserRole` enum 추가 (USER, ADMIN, SUPERUSER)
...
> [!TIP] GitHub Actions에서 모든 push/PR마다 자동으로 테스트가 실행되어 코드 품질을 지속적으로 검증할 수 있음.
> [!TIP] Docker 컨테이너 시작 시 자동으로 마이그레이션이 실행되어 배포 과정에서 수동 작업이 필요 없음.
> [!TIP] Seed 스크립트로 초기 관리자 계정을 쉽게 생성할 수 있으며, 환경 변수로 커스터마이징 가능함.
...
### 3. 다음 액션
- [x] Alembic 자동화(도커 entrypoint 혹은 GH Actions) 설계
- [x] Auth 유즈케이스 단위 테스트 및 API 통합 테스트 작성
- [x] 초기 Admin/Seed 전략 및 사용자 권한(Role) 정의
- [x] CI/CD 파이프라인에 테스트 실행 단계 추가 (GitHub Actions)
